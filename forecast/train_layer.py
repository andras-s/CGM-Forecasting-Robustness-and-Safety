...
horizon = float(sys.argv[1]) if float(sys.argv[1]) == 0.5 else int(sys.argv[1])
seed = int(sys.argv[2])
...
folder_name = f"{args['Layer_hps']['feature_extractor']}_{args.model}/{ft_e2e}_{config['similarity_measure_name']}/seed_{args.seed}"
...
args.save_string = folder_name + f"/horizon_{horizon}_{gp_args.split_attribute}_{time.strftime('%Y%m%d-%H%M%S')}"
