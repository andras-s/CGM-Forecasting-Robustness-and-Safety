...
class TransformerModel(nn.Module):
    ...
    def forward(self, series_id: int, x: torch.Tensor):
        ...
        position = torch.arange(length).clone().detach().to(self.device)
...
class DecoderTransformer(nn.Module):
    def __init__(self, n_time_series: int, n_head: int, num_layer: int,
        ...
        Args:
            ...
            additional_params: Additional parameters used to initialize the attention model. Can inc
        """
